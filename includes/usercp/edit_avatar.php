<?php 
  ...
  if ($module == "")  { 
      ...
      $query_userdata2 = mysql_query("SELECT avatar from $user_tblname WHERE UserID = '$auto_logon_id' && UserPass = '$auto_logon_session'");  
  
      while  ($row_userdata2  =  mysql_fetch_assoc($query_userdata2))  {

              $userdata_avatar  =  $row_userdata2["avatar"];             //  Data:: Avatar
...
*/
      ...
      if ($upload_access == "false") { 
          ...
          $refresh_time = "3"; $link = "index.php?do=usercp&sec=edit_avatar";
      ...
      if (isset($send_data))  {
          ...
          if ($del_avatar == "1")  {
              ...
              $query_av = mysql_query("SELECT avatar from $user_tblname WHERE UserID = '$userdata_id'");

              while ($row_av = mysql_fetch_assoc($query_av))  { 

                    $userdata_avatar = $row_av[avatar];
