<?php 
  ...
  $query_pmNewOne = mysql_query("SELECT * from $pm_in_tblname WHERE receiver_id = '$userdata_id' && status = '0'");
  $newpms2 = mysql_num_rows($query_pmNewOne);
  ...
  $query_newest_posts = mysql_query("SELECT * from $posts_tblname ORDER by time DESC LIMIT 3");
  ...
  while ($row_newest_posts = mysql_fetch_assoc($query_newest_posts))  { 
         ...
         while ($row_n_threads = mysql_fetch_assoc($query_n_threads))  { 
                ...
                $last_thread = $row_n_threads[threadname];

                if(strlen($last_thread) > 30) { 

                   $last_thread = substr($row_n_threads[threadname], 0, 27).'...';
